...
import net.goldgruben.paperapi.utils.FlatWorldGenerator;
...
public class NickAPI {
	...
	public NickAPI() {
		...
		if (online) {
			GoldCore.getInstance().getLogger().log(Level.INFO, "NickAPI wird geladen...");
			try {
				GoldCore.getInstance().getConnection().prepareStatement(
			...
			GoldCore.getInstance().getLogger().log(Level.INFO, "NickAPI wurde NICHT geladen");
		}
	}
	...
	public boolean isNicked(Player player) {
		...
		boolean b = false;
		try {
			PreparedStatement st = GoldCore.getInstance()
			                                .getConnection()
	...
	private void addToDatabase(Player player, String realname, String nickname) throws SQLException {
		if (!online) throw new IllegalStateException("NickAPI not online!");
		PreparedStatement st = GoldCore.getInstance().getConnection().prepareStatement(
